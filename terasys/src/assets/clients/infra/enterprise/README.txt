# Enterprise Client Logos (대기업 고객사 로고)

This folder contains logo images for large enterprise clients (conglomerates, major corporations).
이 폴더에는 대기업 고객사(재벌, 주요 기업)의 로고 이미지가 저장됩니다.
...
- Copy renamed files to this folder: `clients/enterprise/`
- 변경한 파일을 이 폴더에 복사: `clients/enterprise/`

### Step 4: Verify (확인)
- Refresh website / 웹사이트 새로고침
- Navigate to "주요고객" → "대기업" tab / "주요고객" → "대기업" 탭으로 이동
...
| Task | Command/Action |
|------|---------------|
| Check current logos | Count files in `clients/enterprise/` folder |
| Next file number | Last number + 1 |
| Test changes | Refresh browser (Ctrl+F5 / Cmd+Shift+R) |

| 작업 | 명령/동작 |
|------|----------|
| 현재 로고 확인 | `clients/enterprise/` 폴더의 파일 개수 확인 |
| 다음 파일 번호 | 마지막 번호 + 1 |
| 변경사항 테스트 | 브라우저 새로고침 (Ctrl+F5 / Cmd+Shift+R) |

---

**Category**: Enterprise (대기업)
**Folder Path**: `/clients/enterprise/`
**Displayed in**: "주요고객" Section → "대기업" Tab

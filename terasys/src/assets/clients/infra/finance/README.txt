# Finance Client Logos (금융 고객사 로고)

This folder contains logo images for financial institution clients (banks, securities, insurance companies).
이 폴더에는 금융기관 고객사(은행, 증권, 보험)의 로고 이미지가 저장됩니다.
...
- Copy renamed files to this folder: `clients/finance/`
- 변경한 파일을 이 폴더에 복사: `clients/finance/`

### Step 4: Verify (확인)
- Refresh website / 웹사이트 새로고침
- Navigate to "주요고객" → "금융" tab / "주요고객" → "금융" 탭으로 이동
...
| Task | Command/Action |
|------|---------------|
| Check current logos | Count files in `clients/finance/` folder |
| Next file number | Last number + 1 |
| Test changes | Refresh browser (Ctrl+F5 / Cmd+Shift+R) |

| 작업 | 명령/동작 |
|------|----------|
| 현재 로고 확인 | `clients/finance/` 폴더의 파일 개수 확인 |
| 다음 파일 번호 | 마지막 번호 + 1 |
...
**Category**: Finance (금융)
**Folder Path**: `/clients/finance/`
**Displayed in**: "주요고객" Section → "금융" Tab

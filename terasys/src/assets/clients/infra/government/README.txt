# Government Client Logos (관공서 고객사 로고)

This folder contains logo images for government and public sector clients (ministries, agencies, public institutions).
이 폴더에는 관공서 및 공공기관 고객사(부처, 기관, 공기업)의 로고 이미지가 저장됩니다.
...
- Copy renamed files to this folder: `clients/government/`
- 변경한 파일을 이 폴더에 복사: `clients/government/`

### Step 4: Verify (확인)
- Refresh website / 웹사이트 새로고침
- Navigate to "주요고객" → "관공서" tab / "주요고객" → "관공서" 탭으로 이동
...
| Task | Command/Action |
|------|---------------|
| Check current logos | Count files in `clients/government/` folder |
| Next file number | Last number + 1 |
| Test changes | Refresh browser (Ctrl+F5 / Cmd+Shift+R) |

| 작업 | 명령/동작 |
|------|----------|
| 현재 로고 확인 | `clients/government/` 폴더의 파일 개수 확인 |
| 다음 파일 번호 | 마지막 번호 + 1 |
| 변경사항 테스트 | 브라우저 새로고침 (Ctrl+F5 / Cmd+Shift+R) |

---

**Category**: Government (관공서)
**Folder Path**: `/clients/government/`
**Displayed in**: "주요고객" Section → "관공서" Tab
